...
import {
  ...
  Highlight, HorizontalGridLines, VerticalGridLines, XAxis,
  YAxis
} from "react-vis";
import {
  genderLabel,
...
class PensionGraph extends Component {
    ...
    getPensionData(type) {
      ...
      parseData(data){
         ...
         let men = menArray.sort((a, b) => parseFloat(a).toFixed(2) - parseFloat(b).toFixed(2));
         let women = womenArray.sort((a, b) => parseFloat(a).toFixed(2) - parseFloat(b).toFixed(2));
        ...
        for (let i = 0; i < 11; i++) {
          if (i <= 5) {
            ...
            womenGraphObject.push({ x: parseFloat(women[i]).toFixed(2), y: 10 - i });
          }
        }
